...
This repository contains all the necessary actions and instructions on how to deploy a solution based on the Cosmo Tech Platform using the version 3 of Babylon.
  ...
  asset-deploy:
    ...
    steps:
      ...
      - name : 📄 Copy The Templates to The Babylon Env
        run: |
              IFS=',' read -ra platforms_array <<< "dev,perf"
              for platform in "${platforms_array[@]}"; do
                  if [ "$platform" == "${{ env.PLATFORM_ID }}" ]; then
                    cp $config_path/$platform/dataset.yaml ${{ env.dir }}/.payload/${{ env.CONTEXT_ID }}.$platform.dataset.${{ steps.createConnector.outputs.connectorType }}.yaml
                    cp $config_path/$platform/solution.yaml ${{ env.dir }}/.payload/${{ env.CONTEXT_ID }}.$platform.solution.yaml
                  fi
              done
...
```
## Create a new Organization (organization action)
...
The `ADX action` creates an adx databese it depends on the `file.kql` located in the repository where the user runs the workflows, specifically in the `adx` directory.
>run
...
This action will deploy a Power BI workspace. It depends on the Power BI report and scenarios located in the repository where the user runs the workflows, specifically in the `powerbi` directory. The powerbi directory contains two subdirectories, `dashboard` and `scenario`.
>run
...
The `upload_dataset` action uploads all CSV files to a blob container. It depends on all CSV files that are zipped in the `dataset` directory located in the repository where the user runs the workflows.
>run

```bash
- name: Upload dataset
  env:
    DATASET: ${{ github.workspace }}/dataset/${{ env.PLATFORM_ID }}
  run: |
    cd ${{ inputs.folder }}
...
3 handlers are executed in `cloud` mode, meaning outside the Docker image of the simulator: `parameter handler`, `prerun`, and `postrun`. They need to be deployed on the platform. The `upload_handlers` depends on parameter handler, prerun, and postrun, that are zipped in the `handlers` directory located in the repository where the user runs the workflows.
...
- name: Upload handlers
  env:
    HANDLERS: ${{ github.workspace }}/handlers/${{ env.PLATFORM_ID }}
  run: |
    cd ${{ inputs.folder }}
